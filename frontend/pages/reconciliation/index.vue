<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-6">
      <h1 class="text-h4">Сверки</h1>
      <v-btn color="primary" to="/reconciliation/create">
        ...
        >
          <template #item.actions="{ item }">
            <v-btn
              ...
              size="small"
              variant="text"
              :to="`/reconciliation/edit/${item.id}`"
            />
            <v-btn
              icon="mdi-delete"
              size="small"
...
const headers = [
  ...
  { title: 'Наш баланс', key: 'ourBalance' },
  { title: 'Их баланс', key: 'theirBalance' },
...
const fetchItems = async () => {
  loading.value = true
  try {
    const response = await api.get('/api/reconciliations')
...
const deleteItem = async () => {
  ...
  try {
    await api.delete(`/api/reconciliations/${itemToDelete.value.id}`)
